...
class HomeController: UIViewController {
    ...
    private let rideActionView = RideAnctionView()
    ...
    func observeCurrentTrip() {
        PassengerService.shared.observeCurrentTrip { trip in
            ...
            case .denied:
                ...
                self.presentAlertController(withTitle: "Oops", message: "It looks like we coudnt find you a driver. Please try again..")
                PassengerService.shared.deletelTrip { err, ref in
    ...
    func startTrip() {
        ...
        DriverService.shared.updateTripState(trip: trip, state: .inProgress) { err, ref in
            self.rideActionView.config = .tripInProgress
            self.removeAnnotationsAndOverlays()
            self.mapView.addAnnotationsAndSelect(forCoordinates: trip.destinationCoordinates)
    ...
    func observeCancelledTrip(trip: Trip) {
        DriverService.shared.observeTripCancelled(trip: trip) {
            ...
            self.animateRideActionView(shouldShow: false)
            self.centerMapOnUserLocation()
            self.presentAlertController(withTitle: "Oops!", message: "The passenger has canceled this trip")
        }
    }
   ...
   fileprivate func configureActionButton(config: ActionButtonConfiguration) {
        ...
        case .dismissActivationView:
            ...
            actionButtonConfig = .dismissActivationView
        }
    }
...
private extension HomeController {
    ...
    func removeAnnotationsAndOverlays() {
        mapView.annotations.forEach { anotation in
            if let anno = anotation as? MKPointAnnotation {
...
extension HomeController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        configureActionButton(config: .dismissActivationView)
        ...
        dismissLocationView { _ in
            self.mapView.addAnnotationsAndSelect(forCoordinates: selectedPlacemark.coordinate)
...
//MARK: - RideAnctionViewDelegate

extension HomeController: RideAnctionViewDelegate {
    ...
    func cancelTrip() {
        PassengerService.shared.deletelTrip { error, ref in
            if let error = error {
...
extension HomeController: PickupControllerDelegate {
    func didAcceptTrip(_ trip: Trip) {
        ...
        self.mapView.addAnnotationsAndSelect(forCoordinates: trip.pickupCoordinates)
